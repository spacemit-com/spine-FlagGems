...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("bitwise_or_tensor_tensor"),
    key=["n_elements"],
)
@triton.jit
...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("bitwise_or_tensor_scalar"),
    key=["n_elements"],
)
@triton.jit
...
def bitwise_or_tensor(A, B):
    logger.debug("GEMS_SPACEMIT BITWISE_OR")
    A, B = A.contiguous(), B.contiguous()
    out = torch.empty_like(A)
    n = A.numel()
    with torch_device_fn.device(A.device):
        bitwise_or_kernel_tt[(NUM_CTAS,)](A, B, out, n)
    return out


def bitwise_or_tensor_(A, B):
    logger.debug("GEMS_SPACEMIT BITWISE_OR_")
    A_c = A.contiguous()
    B = B.contiguous()
    n = A_c.numel()
    with torch_device_fn.device(A.device):
        bitwise_or_kernel_tt[(NUM_CTAS,)](A_c, B, A_c, n)
...
def bitwise_or_scalar(A, B):
    logger.debug("GEMS_SPACEMIT BITWISE_OR_SCALAR")
    A = A.contiguous()
    out = torch.empty_like(A)
    n = A.numel()
    with torch_device_fn.device(A.device):
        bitwise_or_kernel_ts[(NUM_CTAS,)](A, out, int(B), n)
    return out


def bitwise_or_scalar_(A, B):
    logger.debug("GEMS_SPACEMIT BITWISE_OR__SCALAR")
    A_c = A.contiguous()
    n = A_c.numel()
    with torch_device_fn.device(A.device):
        bitwise_or_kernel_ts[(NUM_CTAS,)](A_c, A_c, int(B), n)
    if not A.is_contiguous():
        A.copy_(A_c)
    return A
...
def bitwise_or_scalar_tensor(A, B):
    logger.debug("GEMS_SPACEMIT BITWISE_OR_SCALAR_TENSOR")
    return bitwise_or_scalar(B, A)

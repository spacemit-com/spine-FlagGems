...
def argmax_kernel(
    ...
    for block_start in range(0, N, BLOCK_N):
        ...
        new_max = tl.maximum(row_max, tl.max(current_block, axis=1))
        mask_eq_max = current_block != new_max[:, None]
        col_indices = col_offsets[None, :]
        candidate_indices = tl.where(mask_eq_max, -1, col_indices)

        new_argmax = tl.max(candidate_indices, axis=1)
        update_mask = new_max > row_max
        row_argmax = tl.where(update_mask, new_argmax, row_argmax)
        row_max = new_max

    out_offsets = row_offsets * K + pid_k
    out_ptrs = out_ptr + out_offsets
    tl.store(out_ptrs, row_argmax, mask=row_mask)

...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("sin"),
    key=["n_elements"],
)
@triton.jit
def sin_kernel(
    ...
    for block_idx in tl.range(0, sub_num):
        ...
        out = sin_func(x.to(tl.float32))
        tl.store(out_blk, out.to(x.dtype), boundary_check=(0,))


def sin(A):
    logger.debug("GEMS_SPACEMIT SIN")
    A = A.contiguous()
    out = torch.empty_like(A)
    n_elements = A.numel()
    with torch_device_fn.device(A.device):
        sin_kernel[(NUM_CTAS,)](A, out, n_elements)
    return out


def sin_(A):
    logger.debug("GEMS_SPACEMIT SIN_")
    A_contig = A.contiguous()
    n_elements = A_contig.numel()
    with torch_device_fn.device(A.device):
        sin_kernel[(NUM_CTAS,)](A_contig, A_contig, n_elements)

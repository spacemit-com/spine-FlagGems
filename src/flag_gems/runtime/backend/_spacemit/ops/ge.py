...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("ge_tensor_tensor"),
    key=["n_elements"],
)
@triton.jit
def ge_kernel_tt(
...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("ge_tensor_scalar"),
    key=["n_elements"],
)
@triton.jit
def ge_kernel_ts(
...
def ge(A, B):
    logger.debug("GEMS_SPACEMIT GE")
    A, B = A.contiguous(), B.contiguous()
    out = torch.empty(A.shape, dtype=torch.uint8, device=A.device)
    n = A.numel()
    ge_kernel_tt[(NUM_CTAS,)](A, B, out, n)
    return out.view(torch.bool)


def ge_scalar(A, B):
    logger.debug("GEMS_SPACEMIT GE_SCALAR")
    A = A.contiguous()
    out = torch.empty(A.shape, dtype=torch.uint8, device=A.device)
    n = A.numel()
    ge_kernel_ts[(NUM_CTAS,)](A, out, float(B), n)
    return out.view(torch.bool)

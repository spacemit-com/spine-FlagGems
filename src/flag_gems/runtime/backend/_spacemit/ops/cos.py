...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("cos"),
    key=["n_elements"],
)
@triton.jit
def cos_kernel(
    ...
    for block_idx in tl.range(0, sub_num):
        ...
        out = cos_func(x.to(tl.float32))
        tl.store(out_blk, out.to(x.dtype), boundary_check=(0,))


def cos(A):
    logger.debug("GEMS_SPACEMIT COS")
    A = A.contiguous()
    out = torch.empty_like(A)
    n_elements = A.numel()
    with torch_device_fn.device(A.device):
        cos_kernel[(NUM_CTAS,)](A, out, n_elements)
    return out


def cos_(A):
    logger.debug("GEMS_SPACEMIT COS_")
    A_contig = A.contiguous()
    n_elements = A_contig.numel()
    with torch_device_fn.device(A.device):
        cos_kernel[(NUM_CTAS,)](A_contig, A_contig, n_elements)

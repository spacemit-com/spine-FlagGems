...
def all_kernel_1(
    ...
    tl.store(mid_ptr, all_val.to(tl.int8), boundary_check=(0,))
...
def all_kernel_2(mid, out, MID_SIZE, BLOCK_MID: tl.constexpr):
    ...
    all_val = tl.reduce((mid_val != 0), axis=0, combine_fn=reduce_all)
    ...
    tl.store(out_ptr, all_val.to(tl.int8), boundary_check=(0,))

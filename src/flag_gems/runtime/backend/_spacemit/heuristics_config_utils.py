...
def argmax_heur_divisible_n(args):
    return args["N"] == args["TILE_N"]
...
def batch_norm_heur_block_n(args):
    ...
    max_block_n = max(1, 2**14 // BLOCK_M)
    if args["batch_dim"] <= 16 and args["spatial_dim"] <= 2048:
        max_block_n = min(max_block_n, 256)
    return min(BLOCK_N, max_block_n)
...
HEURISTICS_CONFIGS = {
    ...
    "argmax_inner_spacemit": {
        "TILE_N": argmax_heur_tile_n_inner,
        "ONE_TILE_PER_CTA": argmax_heur_one_tile_per_cta,
        "DIVISIBLE_N": argmax_heur_divisible_n,
        "num_warps": argmax_heur_num_warps_inner,
    },
    "argmin": {

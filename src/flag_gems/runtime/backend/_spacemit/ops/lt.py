...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("lt_tensor_tensor"),
    key=["n_elements"],
)
@triton.jit
def lt_kernel_tt(
...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("lt_tensor_scalar"),
    key=["n_elements"],
)
@triton.jit
def lt_kernel_ts(
...
def lt(A, B):
    logger.debug("GEMS_SPACEMIT LT")
    A, B = A.contiguous(), B.contiguous()
    out = torch.empty(A.shape, dtype=torch.uint8, device=A.device)
    n = A.numel()
    lt_kernel_tt[(NUM_CTAS,)](A, B, out, n)
    return out.view(torch.bool)


def lt_scalar(A, B):
    logger.debug("GEMS_SPACEMIT LT_SCALAR")
    A = A.contiguous()
    out = torch.empty(A.shape, dtype=torch.uint8, device=A.device)
    n = A.numel()
    lt_kernel_ts[(NUM_CTAS,)](A, out, float(B), n)
    return out.view(torch.bool)
